...
const Home = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (fileData.length === 0 || fileData.some(item => !item.company)) {
      alert('Please upload at least one file and select a company for each file');
      return;
    }
    ...
    <div style={{
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <div style={{ marginBottom: '20px' }}>
          <label htmlFor="fileUpload" style={{ display: 'block', marginBottom: '10px', fontWeight: 'bold' }}>Upload Audit Files:</label>
          <input
            type="file"
            id="fileUpload"
        ...
        <div>
          <button
            type="submit"
            disabled={fileData.length === 0 || fileData.some(item => !item.company)}
            style={{
              width: '100%',
              padding: '12px',
              backgroundColor: fileData.length === 0 || fileData.some(item => !item.company) ? '#ccc' : '#4a90e2',
              color: 'white',
              border: 'none',
              borderRadius: '5px',
              fontSize: '16px',
              cursor: fileData.length === 0 || fileData.some(item => !item.company) ? 'not-allowed' : 'pointer'
            }}
          >
            Upload and Process
          </button>
